...
**件数は環境によって変わります。** Windowsでは PS-07（ファイル権限の確認）と PS-25（Windows以外での `-Execute` 拒否）が対象外になり、スキップした旨を1行だけ記録します。また PS-03 は「同じ場所を指す別の書き方」をOSごとに変えて試すため、確認する件数自体が変わります。**件数が違うのは不具合ではなく、「この環境では何を確認し、何を対象外にしたか」を正直に記録した結果です。**

コミット `b28d3c4` での実測値は次のとおりです。

| 実行環境 | 実測 |
|---|---|
| Linux（PowerShell 7.4.6） | `1..66` / `pass=66 fail=0` |
| GitHub Actions ubuntu-latest | Linuxと同じ |
| GitHub Actions windows-latest | `1..63` / `pass=63 fail=0` |

各コミットでの実測値は[検証証跡](08-evidence.md)に記録します。テストを追加すれば件数は増え、OSによっても変わるので、**数字そのものではなく「`fail=0`」を合格条件**にしてください。
...
PS-07とPS-25は、実行環境がWindowsのときはスキップし、その旨を結果に残します（黙って消しません）。PS-03・PS-15・PS-24・PS-30は、OSによって「危険なパス」「正しい区切り文字」の形が違うため、実行環境に合わせて判定対象を切り替えます。

PS-30は、CIのWindowsジョブで実際に見つかった不具合の回帰テストです。Windowsは `\` と `/` のどちらも区切り文字として通してしまうため、`C:\ops-lab\source/inner` のような混在した書き方をすると「保存先が保存元の中にある」という検査をすり抜けていました。Linuxでは再現しない不具合で、**windows-latest でもテストを回していたから見つかりました。**
