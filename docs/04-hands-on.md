...
ここまでの5問は「読みながら1行ずつ確認する」ための入口です。同じ題材を、採点付きで21問に広げたものが[Bash演習案件パック](../exercises/README.md)です。上の演習は次の演習IDに対応します。

| このページ | 演習パック | 違い |
|---|---|---|
| 演習1（終了コード） | `E01` | 結果をファイルに書き、機械が照合します |
| 演習3（バックアップを復元） | `E19` | 既存テストと同じ5ケースに合格するまで採点します |
| 演習4（わざと失敗させる） | `E09` | 8ケースの拒否理由を模範解答と突き合わせます |
| 演習5（PythonでJSON証跡） | `E17` | `result` と `counts` の中身まで判定します |

上の「まず覚える6記号」の表は正典です。演習パックは記号カードを新設せず、ここを参照します。

```bash
bash exercises/labctl.sh init
bash exercises/labctl.sh show E01
```
